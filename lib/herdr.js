...
export function createHerdrApi(cfg) {
   ...
   */
  async function wake({ provider = 'minimax', cwd = null } = {}) {
    ...
    const hostPane = await resolveHostPane();
    if (!hostPane) throw new Error('无法定位 herdr 宿主 pane');
    const newPane = await splitRight(hostPane, workCwd);
    ...
    await rename(newPane, cfg.paneName);
