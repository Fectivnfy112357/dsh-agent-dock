...
// v0.4.14：缓存 mcode pane 的 viewport 大小（cols + rows）。wake 流程在 zoom 后从响应里
// 提取 layout.area.width/height 写入 cache；status 端点读 cache 返回给 xterm 客户端，
// 让 xterm 网格 cols 数跟 mcode pane viewport_cols 同步，字符位置对齐（避免 mcode 按
// workspace viewport_cols 输出但 xterm 按 details column 宽度显示导致的字符错乱/竖向排列）。
const viewportCache = { cols: null, rows: null };

/** 运行一次 herdr CLI（超时即 kill；Windows shim 回退）。 */
export function runCli(bin, args, { timeoutMs = 15000, cwd } = {}) {
  return new Promise((resolvePromise) => {
    // v1.2 DEBUG: 把每次 herdr CLI 调用写到日志文件（console.error 在 dsh web stderr 不被捕获）
    try {
      const fs = require('node:fs');
      const os = require('node:os');
      const path = require('node:path');
      const logFile = path.join(os.tmpdir(), 'agent-dock-herdr.log');
      fs.appendFileSync(logFile, `[${new Date().toISOString()}] spawn bin=${bin} spawnCwd=${cwd || '(inherit '+process.cwd()+')'} args=${JSON.stringify(args)}\n`);
    } catch (e) { /* ignore */ }
    ...
    function attemptNext(index) {
      const spawnOpts = { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] };
      if (cwd) spawnOpts.cwd = cwd;
      const child = spawn(candidates[index] ?? bin, args, spawnOpts);
      let stdout = '';
      let stderr = '';
      let settled = false;
...
export function createHerdrApi(cfg, deps = {}) {
  ...
  /**
   * 终端面板读：拿 agent pane 的当前屏幕，--format=ansi 让 xterm.js 能渲染颜色/光标。
   * 默认 source=recent（保留 alternate screen 与 ANSI 转义）；format=ansi 时 herdr fork 0.8.0
   * 输出完整 VT 序列（\x1b[38;2;R;G;Bm 真颜色 / \x1b[<row>;<col>H 光标定位 / \x1b[7m 反显）。
   * @param {string} target agent / pane id（agent_wake 后的 pane_id 或 agent name）
   * @param {object} opts { lines=60, source='recent', format='ansi' }
   */
  async function terminalRead(target, { lines = 60, source = 'recent', format = 'ansi' } = {}) {
    const args = ['agent', 'read', target, '--source', source, '--lines', String(lines), '--format', format];
    const r = await runCli(bin, args, { timeoutMs: 10000 });
    if (r.code !== 0) return { ok: false, error: { code: 'exit', message: r.stderr || r.stdout }, text: '' };
    return { ok: true, error: null, text: r.stdout };
  }

  /**
   * 终端面板：向 pane 发送字面文本（herdr pane send-text 透传，无需键名翻译）。
   * 用于普通对话/粘贴/补全——mcode 会按自身 TUI 协议解析。
   */
  async function terminalSendText(paneId, text) {
    if (!paneId) return { ok: false, error: { code: 'bad_arg', message: 'paneId required' } };
    if (typeof text !== 'string' || text.length === 0) return { ok: false, error: { code: 'bad_arg', message: 'text required' } };
    if (text.length > 8000) return { ok: false, error: { code: 'too_long', message: 'text must be <= 8000 chars' } };
    if (text.startsWith('-')) return { ok: false, error: { code: 'bad_arg', message: 'text must not start with -' } };
  ...
  async function splitRight(hostPaneId, cwd, { direction = 'right' } = {}) {
    ...
    // v1.2：spawn herdr CLI 时显式传 cwd，让 herdr server 进程也在 workCwd 下运行，
    // 确保后续 pane run 启动的 mcode 继承正确 cwd（herdr fork 0.8.0 pane split 的
    // --cwd 在某些版本下不覆盖 mcode 进程的初始 cwd，需要从 spawn 层补强）。
    const r = await run(args, cwd ? { cwd } : undefined);
    return r.ok ? (r.data?.pane?.pane_id ?? null) : null;
  }

  async function paneRun(paneId, command, cwd) {
    // v1.2：透传 cwd 给 herdr CLI 调用，确保 mcode 启动进程在 workCwd 下。
    return run(['pane', 'run', paneId, command], cwd ? { cwd } : undefined);
  }
  ...
  async function wake({ provider = 'minimax', cwd = null, hostPane = null } = {}) {
    ...
    // v1.2 DEBUG：记录 wake 入口的关键参数
    console.error('[agent-dock DEBUG wake] entry provider=' + provider + ' cwd=' + JSON.stringify(cwd) + ' hostPane=' + JSON.stringify(hostPane) + ' workCwd=' + JSON.stringify(workCwd) + ' cfg.mcodeCwd=' + JSON.stringify(cfg.mcodeCwd) + ' process.cwd=' + JSON.stringify(process.cwd()));
    ...
    console.error('[agent-dock DEBUG wake] owned=' + JSON.stringify(owned ? { pane_id: owned.pane_id, cwd: owned.cwd, name: owned.name } : null));
    if (owned) {
    ...
    console.error('[agent-dock DEBUG wake] hostPaneId=' + JSON.stringify(hostPaneId));
    if (!hostPaneId) throw new Error('无法定位 herdr 宿主 pane');
    console.error('[agent-dock DEBUG wake] splitRight(' + hostPaneId + ', ' + workCwd + ')');
    const newPane = await splitRight(hostPaneId, workCwd);
    console.error('[agent-dock DEBUG wake] newPane=' + JSON.stringify(newPane));
    if (!newPane) throw new Error('pane 分裂失败');
    // v0.4.13：让 mcode pane zoom 到 workspace viewport。
    // splitRight 默认 ratio=0.5 让 mcode pane viewport_rows = workspace_height/2 ≈ 25 行，
    // mcode 按 25 行屏幕布局（~24 行内容顶部对齐），xterm 网格（视口高度/行高 ≈ 50+ 行）
    // 下方大片留白。zoom 让 mcode pane 占满 workspace viewport，mcode 按大屏幕输出
    // 更多内容（Tips for getting started + What's new 等），xterm 网格被填满。
    // v0.4.14：zoom 响应里 layout.area.width/height 是 mcode pane 真实 viewport 大小，
    // 写入 cache 让 status 端点返回给 xterm 客户端，xterm 网格 cols 数跟 viewport_cols
    // 同步，字符位置对齐。
    try {
      const zoomRes = await run(['pane', 'zoom', newPane, '--on']);
      if (zoomRes.ok && zoomRes.data && zoomRes.data.layout) {
        var area = zoomRes.data.layout.area;
        if (area && area.width) viewportCache.cols = area.width;
        if (area && area.height) viewportCache.rows = area.height;
      }
    } catch (e) { console.warn('[agent-dock] zoom 失败（不影响 wake）: ' + (e?.message ?? e)); }
    const cmd = p.startupCommand(cfg, workCwd);
    console.error('[agent-dock DEBUG wake] paneRun(' + newPane + ', ' + JSON.stringify(cmd) + ', cwd=' + workCwd + ')');
    const runRes = await paneRun(newPane, cmd, workCwd);
    console.error('[agent-dock DEBUG wake] runRes=' + JSON.stringify(runRes));
  ...
  return {
    ...
    terminalRead, terminalSendText,
    wake, stop,
    // v0.4.14：返回 mcode pane 的 viewport 大小（cols + rows，wake 时从 zoom 响应写 cache）
    getViewport: function () { return Object.assign({}, viewportCache); },
