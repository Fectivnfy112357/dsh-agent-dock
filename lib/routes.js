...
function buildStatusPayload(api, agents, panes, cwd) {
  ...
  // v0.4.14：从 cache 读 mcode pane viewport 大小，加到 mine 字段，xterm 客户端按
  // viewport_cols 渲染 xterm 网格 cols 数（避免字符按 mcode 输出宽度错位）。
  const viewport = (typeof api.getViewport === 'function') ? api.getViewport() : { cols: null, rows: null };
  return {
    ok: true,
    ...
    terminalPollMs: api.cfg.terminalPollMs ?? 1500,
    serverOk: true,
    forkOk: true,
    agents: summarized,
    mine: owned ? Object.assign(summarizeOwned(owned), {
...
export function mountRoutes(host, api) {
  ...
  push({
    ...
    handler: async (request, response) => {
      ...
      // v1.2 DEBUG: 把 HTTP route 触发写到日志
      try {
        const fs = require('node:fs');
        const os = require('node:os');
        const path = require('node:path');
        const logFile = path.join(os.tmpdir(), 'agent-dock-route.log');
        fs.appendFileSync(logFile, `[${new Date().toISOString()}] /agent-dock/wake hit process.cwd=${process.cwd()}\n`);
      } catch (e) { /* ignore */ }
      try {
  ...
  push({
    kind: 'exact',
    path: '/agent-dock/terminal/text',
    handler: async (request, response) => {
      if (request.method !== 'POST') { response.writeHead(405, { allow: 'POST' }); response.end(); return; }
      if (!sameOrigin(request)) { sendJson(response, 403, { ok: false, error: { code: 'origin', message: 'cross-origin request rejected' } }); return; }
      try {
        const body = await readJsonBody(request).catch(() => ({}));
        const lines = cleanTimeout(body.lines ?? 80, 80, 500);
        const source = ['visible', 'recent', 'recent-unwrapped', 'detection'].includes(body.source) ? body.source : 'recent';
        const format = body.format === 'text' ? 'text' : 'ansi';
        const target = await resolveOpTarget(api, body.cwd);
        if (!target) { sendJson(response, 409, { ok: false, error: { code: 'agent_not_owned', message: '未检测到归属的 agent，请先唤醒' } }); return; }
        const r = await api.terminalRead(target, { lines, source, format });
        if (!r.ok) { sendJson(response, 502, { ok: false, error: r.error, text: '' }); return; }
        sendJson(response, 200, { ok: true, target, format, lines, source, text: r.text });
      } catch (error) {
        sendJson(response, 500, { ok: false, error: { code: 'terminal_text', message: String(error?.message ?? error) } });
      }
    },
  });
  ...
  push({
    ...
    handler: async (request, response) => {
      ...
      try {
        ...
        const target = await resolveOpTarget(api, body.cwd);
        if (!target) { sendJson(response, 409, { ok: false, error: { code: 'agent_not_owned', message: '未检测到归属的 agent，请先唤醒' } }); return; }
        // 文本：走 pane send-text（需把 agent 名解析为 pane_id）
        if (typeof body.text === 'string' && body.text.length > 0) {
          if (body.text.length > 8000) { sendJson(response, 400, { ok: false, error: { code: 'too_long', message: 'text must be <= 8000 chars' } }); return; }
          let paneId = target;
          const detail = await api.get(target).catch(() => null);
          if (detail && detail.ok && detail.data && detail.data.agent && detail.data.agent.pane_id) paneId = detail.data.agent.pane_id;
          const r = await api.terminalSendText(paneId, body.text);
          if (!r.ok) { sendJson(response, 502, { ok: false, error: r.error }); return; }
          sendJson(response, 200, { ok: true, channel: 'text', pane: paneId });
          return;
        }
        // 键：走 agent send-keys（target 是 agent 形式即可）
        if (Array.isArray(body.keys) && body.keys.length > 0) {
          const cleaned = body.keys.filter((k) => typeof k === 'string' && k.length > 0 && k.length <= 64).slice(0, 16);
          if (cleaned.length === 0) { sendJson(response, 400, { ok: false, error: { code: 'keys', message: 'no valid keys' } }); return; }
          const r = await api.sendKeys(target, cleaned);
          if (!r.ok) { sendJson(response, 502, { ok: false, error: r.error }); return; }
          sendJson(response, 200, { ok: true, channel: 'keys', keys: cleaned });
          return;
        }
        sendJson(response, 400, { ok: false, error: { code: 'bad_body', message: 'either text or keys required' } });
      } catch (error) {
        sendJson(response, 500, { ok: false, error: { code: 'terminal_send', message: String(error?.message ?? error) } });
      }
    },
  });
