 ...
 * human/unknown 停止自动推进并返回对话框内容供 DSH 转人工。
 */
async function autoApproveBlocked(api) {
  const read = await api.read(api.cfg.paneName, { source: 'recent-unwrapped', lines: 40 });
  const dialog = read.ok ? read.text : '';
  const cls = classifyBlocked(dialog);
  if (cls === 'auto' && shouldAutoApprove(cls, api.cfg)) {
    await api.sendKeys(api.cfg.paneName, 'enter');
    await new Promise((resolve) => setTimeout(resolve, 800));
    const after = await api.wait(api.cfg.paneName, { timeoutMs: Math.min(api.cfg.delegationTimeoutMs, 120000) });
  ...
  return [
    ...
    },
    {
      ...
      execute: async (args) => toToolResult(async () => {
        const r = await api.prompt(P(), args.text, { wait: args.wait ?? true, until: args.until ?? [], timeoutMs: args.timeoutMs });
        const blocked = args.wait === false ? false : (!r.ok && r.error?.code === 'agent_blocked') || (r.ok && !!r.data?.agent && stateOf(r.data.agent) === 'blocked');
        if (blocked) return autoApproveBlocked(api);
        return settled(r);
      }),
    },
    {
      ...
      output: { schema: { type: 'json' }, render: jsonTextRender },
      execute: async (args) => toToolResult(async () => settled(await api.wait(P(), { until: args.until ?? [], timeoutMs: args.timeoutMs ?? 120000 }))),
    },
    {
      name: 'agent_read',
