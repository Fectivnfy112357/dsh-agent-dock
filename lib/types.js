...
export const DEFAULT_CONFIG = {
  ...
  /** WebUI 徽章轮询间隔（ms）。 */
  pollIntervalMs: 2000,
  /** WebUI 右侧终端面板轮询间隔（ms）；越小越接近 herdr TUI 实时感，但 herdr fork 0 的 agent read 是阻塞式 spawn —— 太短会起进程风暴。 */
  terminalPollMs: 1500,
  /** 单次委托超时（ms），默认 15 分钟。 */
...
export function validateConfig(cfg) {
  ...
  if (cfg.terminalPollMs !== undefined && (!Number.isFinite(cfg.terminalPollMs) || cfg.terminalPollMs < 250 || cfg.terminalPollMs > 60000)) errors.push('terminalPollMs must be between 250 and 60000');
