...
export function apply(ctx, config) {
  ...
  // 3) 动态工具（DSH agent 全自动协同）——harness 为宿主沙箱全局（不是服务）
  const harnessGlobal = (typeof globalThis !== 'undefined' && globalThis.harness) || (typeof harness !== 'undefined' ? harness : null);
  if (!harnessGlobal || typeof harnessGlobal.defineTool !== 'function' || typeof harnessGlobal.registerTool !== 'function') {
    throw new Error('dsh-agent-dock: harness sandbox global unavailable (defineTool/registerTool missing)');
  }
  for (const def of buildToolDefs(api)) {
    ctx.effect(() => harnessGlobal.registerTool(ctx, harnessGlobal.defineTool(def)), 'agent-dock: tool ' + def.name);
  }
}
